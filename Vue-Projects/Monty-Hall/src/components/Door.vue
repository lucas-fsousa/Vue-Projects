<template>
  <div class="door-area">
    <div class="door-frame" :class="{ selected: selected && !opened }">
      <Gift v-if="hasGift && opened"/>
    </div>
    <div class="door" :class="{ opened }" @click="selected = !selected">
      <div class="number" :class="{ selected }">{{ number }}</div>
      <div class="knob" :class="{ selected }" @click.stop="opened = true"></div>
    </div>
  </div>
</template>
